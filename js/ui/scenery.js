...
let scenerySig="",prevOwn={};BUILDINGS.forEach(b=>prevOwn[b.id]=0);
...
export function initSceneryTooltip(){
  scenery.addEventListener("mousemove",e=>{const band=e.target.closest(".band");if(!band){tip.style.display="none";return;}
    ...
    const b=BUILDINGS.find(x=>x.id===band.dataset.id),owned=s.own[b.id],per=b.cps*up.bld[b.id]*state.curMult,tot=owned*per,pct=s.cps>0?(tot/s.cps*100):0,ups=upgForBld(b.id),low=LOWIDS.indexOf(b.id)>=0,perkMul=up.bld[b.id]/Math.pow(2,ups);
