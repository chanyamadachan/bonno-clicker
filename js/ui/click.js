...
import { bong, pok, critSfx, chant } from "../core/audio.js";
import { HEART } from "../data/content.js";
...
zone.addEventListener("click",e=>{
  ...
  if(state.lastTap){const gap=t-state.lastTap;
    if(gap>=190&&gap<=820){state.tapGaps.push(gap);if(state.tapGaps.length>5)state.tapGaps.shift();const avg=state.tapGaps.reduce((a,c)=>a+c,0)/state.tapGaps.length;if(Math.abs(gap-avg)<=155)state.combo=Math.min(up.comboMax,state.combo+1);else state.combo=Math.max(0,state.combo-2);}
    else{state.combo=Math.max(0,state.combo-3);state.tapGaps.length=0;}}
  ...
  if(state.combo>=10&&Math.random()<0.55){spawnSutra();chant();}
